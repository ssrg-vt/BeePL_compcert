...
extern signed char const __stringlit_0[20];
...
int xdp_prog(struct xdp_md *p)
{
  ...
  pid = (int) (pid_tgid & (unsigned long long) 4294967295LL);
  tgid =
    (int) ((unsigned long long) 32 < 64LLU
            ? pid_tgid >> (unsigned long long) 32 : 0LLU);
  r = bpf_printk(__stringlit_0, pid, tgid);
  return 2;
}

signed char const __stringlit_0[20] = "PID: %d, TGID: %d\134n";
