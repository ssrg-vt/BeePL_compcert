...
| ty_addr : forall Gamma Sigma l ofs h t a,
            PTree.get l.(lname) Sigma = Some l.(ltype) ->
            t = (Reftype h l.(ltype) a) ->
...
Context (Htloc : forall Gamma Sigma l h t a ofs, 
                 PTree.get l.(lname) Sigma = Some l.(ltype)  ->
                 t = (Reftype h l.(ltype) a) ->
...
+ by move=> Gamma Sigma l h t a ofs hs hl ef' t' ht; inversion ht; subst.
+ by move=> Gamma Sigma efs' ts' ht; inversion ht; subst.
...
(* A store st is well-typed with respect to a store typing context Sigma if the
   term at each location l in st has the type at location l in ST *)
Definition store_well_typed (Sigma : store_context) (vm : vmap) :=
forall l t, 
exists v, vm! l = Some(v, (Ptype t)) /\
...
Proof.
suff : (forall Gamma Sigma es efs ts, type_exprs Gamma Sigma es efs ts ->
                                      forall bge vm m, store_well_typed Sigma vm ->
                                                       is_values es \/ exists m' vm' es',
                                                       ssem_exprs bge vm m es m' vm' es') /\
(forall Gamma Sigma e ef t, type_expr Gamma Sigma e ef t ->
                            forall bge vm m, store_well_typed Sigma vm ->
...
(* lvar *)
+ move=> Gamma Sigma v t ht hteq bge vm m hw; subst. right.
  rewrite /store_well_typed in hw. 
  exists m. exists vm. eexists. eapply ssem_lvar. + admit.
  admit. 
(* gvar *)
+ admit.
(* const long *)
...
(* addr *)
+ move=> Gamma Sigma l h t a ofs hs ht bge vm m vm' m' v he.
...
+ rewrite H0 in H5. by case: H5=> h1 h2 h3; subst.
+ rewrite H1 in H6. by case: H6=> h1 h2 h3; subst.
by inversion H7; subst. 
Qed.
