...
| EF_external of string
   * signature
| EF_builtin of string
   * signature
| EF_runtime of string
   * signature
...
| Tint0 of intsize
   * signedness * 
   attr
| Tlong0 of signedness
   * attr
| Tfloat0 of floatsize
   * attr
| Tpointer of type0 * attr
| Tarray of type0 * 
   z * attr
| Tfunction of typelist
   * type0
   * calling_convention
...
prog_main : ident;
prog_types : composite_definition
             list;
...
| Eassign of expr * 
   expr * type0
| Eassignop of binary_operation
   * expr * expr * 
   type0 * type0
| Epostincr of incr_or_decr
   * expr * type0
| Ecomma of expr * 
   expr * type0
| Ecall of expr * 
   exprlist * type0
| Ebuiltin of external_function
   * typelist * exprlist
   * type0
| Eloc of block
   * Ptrofs.int * 
...
prog_main0 : ident;
prog_types0 : composite_definition
              list;
prog_comp_env0 : composite_env }

(** val transBeePL_value_cvalue :
    value -> val0 **)
...
| Cons (e, es0) ->
  bind0
    (transBeePL_expr_expr0
      e) (fun ce ->
    bind0
      (transBeePL_expr_exprs
        transBeePL_expr_expr0
        es0) (fun ces ->
      ret (Econs (ce, ces))))

(** val exprlist_list_expr :
    exprlist -> expr list **)
...
let transBeePL_function_function fd =
  match transBeePL_type
          fd.fn_return0
          (initial_generator
            Tt) with
  | Err msg -> Error msg
  | Res (crt, _) ->
    (match transBeePL_types
             transBeePL_type
             (unzip2
               (extract_list_rvtypes
                 fd.fn_args))
...
(** val transBeePL_fundef_fundef :
    fundef1 -> fundef0 res **)

let transBeePL_fundef_fundef = function
| Internal0 f ->
  bind
    (transBeePL_function_function
      f) (fun tf -> OK
    (Internal tf))
| External0 ->
  Error (Cons ((MSG
...
| Gfun f ->
  bind
    (transBeePL_fundef_fundef
      f) (fun cf -> OK
    (Gfun cf))
| Gvar g ->
  bind
    (transBeePLglobvar_globvar
      g) (fun cg -> OK
    (Gvar cg))
