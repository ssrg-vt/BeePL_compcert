  ...
  let quotrem a b =
    ...
    | Zpos a0 ->
      (match b with
       | Z0 -> Pair (Z0, a)
       | Zpos b0 ->
         let Pair (q, r) = N.pos_div_eucl a0 (Npos b0) in Pair ((of_N q), (of_N r))
       | Zneg b0 ->
  ...
  let coq_lor a b =
    ...
    | Zneg a0 ->
      (match b with
       ...
       | Zneg b0 ->
         Zneg (N.succ_pos (N.coq_land (Coq_Pos.pred_N a0) (Coq_Pos.pred_N b0))))
...
module Make =
 ...
 struct
  ...
  let divmods2 nhi nlo d =
    ...
    | Right ->
      let Pair (q, r) =
        Z.quotrem (Z.add (Z.mul (signed nhi) modulus) (unsigned nlo)) (signed d)
      in
...
type 'v globvar = { gvar_info : 'v; gvar_init : init_data list; gvar_readonly : 
                    bool; gvar_volatile : bool }
...
type composite = { co_su : struct_or_union; co_members : members; co_attr : attr;
                   co_sizeof : z; co_alignof : z; co_rank : nat }
...
type function0 = { fn_return : type0; fn_callconv : calling_convention;
                   ...
                   fn_vars : (ident, type0) prod list; fn_body : statement }
...
let gensym ty g =
  Res (g.gen_next, { gen_next = (Coq_Pos.succ g.gen_next); gen_trail = (Cons ((Pair
...
let bsig_to_csig bsig =
  bind0 (transBeePL_types transBeePL_type bsig.bsig_args) (fun cts ->
    bind0 (transBeePL_type bsig.bsig_res) (fun ct ->
      ret { sig_args = (map0 typ_of_type (from_typelist cts)); sig_res =
        (rettype_of_type ct); sig_cc = bsig.bsig_cc }))

type external_function0 =
| EF_external0 of string * bsignature

(** val befunction_to_cefunction : external_function0 -> external_function mon **)

let befunction_to_cefunction = function
| EF_external0 (n0, bsig) ->
  bind0 (bsig_to_csig bsig) (fun aef -> ret (EF_external (n0, aef)))
...
type function1 = { fn_return0 : type1; fn_effect : effect;
                   fn_callconv0 : calling_convention;
                   fn_args : (ident, type1) prod list;
                   fn_vars0 : (ident, type1) prod list; fn_body0 : expr0 }
...
| Cons (e, es0) ->
  bind0 (transBeePL_expr_expr0 e) (fun ce ->
    bind0 (transBeePL_expr_exprs transBeePL_expr_expr0 es0) (fun ces ->
      ret (Econs (ce, ces))))
...
| Val (v, t0) ->
  bind0 (transBeePL_type t0) (fun vt -> ret (Eval ((transBeePL_value_cvalue v), vt)))
...
| Const (c, t0) ->
  (match c with
   ...
   | ConsUnit ->
     bind0 (transBeePL_type t0) (fun ut -> ret (Eval ((Vint (Int.repr Z0)), ut))))
| App (e0, es, t0) ->
       ...
       bind0 (transBeePL_type t0) (fun ct ->
         bind0 (gensym ct) (fun tv ->
           ret (Ecomma ((Eassign ((Evar (tv, ct)),
             (hd default_expr (exprlist_list_expr ces)), ct)), (Eaddrof ((Evar (tv,
             ct)), ct)), ct)))))
   | Deref ->
     bind0 (transBeePL_expr_exprs transBeePL_expr_expr es) (fun ces ->
       bind0 (transBeePL_type t0) (fun ct ->
...
| Eapp (ef, ts, es, t0) ->
  bind0 (befunction_to_cefunction ef) (fun cef ->
         ...
         bind0 (transBeePL_type t0) (fun ct ->
           bind0 (transBeePL_type t') (fun _ ->
             bind0 (transBeePL_type (typeof_expr e')) (fun rt ->
               ret (Ssequence ((Sdo (Eassign ((Evar (x, ct)), ce, Tvoid0))), (Sreturn
                 (Some (Evalof (ce', rt)))))))))))
   | Const (_, t1) ->
     bind0 (transBeePL_type t1) (fun ct ->
       bind0 (transBeePL_expr_expr e0) (fun ce ->
         bind0 (transBeePL_expr_expr e') (fun ce' ->
           ret (Ssequence ((Sdo (Eassign ((Evar (x, ct)), ce, Tvoid0))), (Sreturn (Some
             ce')))))))
   | _ ->
     bind0 (transBeePL_type t0) (fun ct ->
       bind0 (transBeePL_expr_expr e0) (fun ce ->
      ...
      bind0 (transBeePL_expr_expr e'') (fun ce'' ->
        bind0 (transBeePL_type t') (fun ct' ->
          match match check_var_const e' with
                ...
                | False -> False with
          | True ->
            ret (Sifthenelse (ce, (Sreturn (Some (Evalof (ce', ct')))), (Sreturn (Some
              (Evalof (ce'', ct'))))))
          | False ->
            (match check_var_const e' with
             | True ->
               ret (Sifthenelse (ce, (Sreturn (Some (Evalof (ce', ct')))), (Sdo ce'')))
             | False ->
               (match check_var_const e'' with
                | True ->
                  ret (Sifthenelse (ce, (Sdo ce'), (Sreturn (Some (Evalof (ce'',
                    ct'))))))
...
let transBeePL_function_function fd =
  ...
  | Res (crt, _) ->
    (match transBeePL_types transBeePL_type (unzip2 fd.fn_args) (initial_generator Tt) with
     | Err msg -> Error msg
     | Res (pt, _) ->
       (match transBeePL_types transBeePL_type (unzip2 fd.fn_vars0)
...
| External0 (ef, ts, t0, cc) ->
  (match befunction_to_cefunction ef (initial_generator Tt) with
   | Err msg -> Error msg
   | Res (cef, _) ->
...
let transBeePLglobvar_globvar gv =
  ...
  | Err msg -> Error msg
  | Res (gvt, _) ->
    OK { gvar_info = gvt; gvar_init = gv.gvar_init; gvar_readonly = gv.gvar_readonly;
...
let beePL_compcert p =
  bind (transBeePL_globdefs_globdefs (unzip2 p.prog_defs0)) (fun pds -> OK
    { prog_defs = (zip (unzip1 p.prog_defs0) pds); prog_public = p.prog_public0;
    prog_main = p.prog_main0; prog_types = p.prog_types0; prog_comp_env =
    p.prog_comp_env0 })
...
let transf_beepl_program_csyntax p =
  apply_partial (OK p)
    (time (String ((Ascii (True, True, False, False, False, False, True, False)),
      (String ((Ascii (True, True, False, False, True, True, True, False)), (String
      ((Ascii (True, False, False, True, True, True, True, False)), (String ((Ascii
      (False, True, True, True, False, True, True, False)), (String ((Ascii (False,
      False, True, False, True, True, True, False)), (String ((Ascii (True, False,
      False, False, False, True, True, False)), (String ((Ascii (False, False, False,
      True, True, True, True, False)), (String ((Ascii (False, False, False, False,
      False, True, False, False)), (String ((Ascii (True, True, True, False, False,
      ...
      True, True, False, True, True, False)), (String ((Ascii (False, True, True, True,
...
let _main =
  XO (XI (XI (XO (XI (XO (XO (XI (XO (XI (XO (XO (XO (XI (XO (XO (XI (XO (XI (XI (XI
    (XO (XI (XO XH)))))))))))))))))))))))

(** val f_main : function1 **)

let f_main =
  { fn_return0 = (Ptype (Tlong1 (Unsigned, dattr))); fn_effect = Nil; fn_callconv0 =
    cc_default; fn_args = Nil; fn_vars0 = Nil; fn_body0 = (Const ((ConsLong
    (Int64.repr (Zpos (XI (XO XH))))), (Ptype (Tlong1 (Unsigned, dattr))))) }
...
(** val return_example1 : program1 **)

let return_example1 =
  { prog_defs0 = global_definitions; prog_public0 = public_idents; prog_main0 = _main;
    prog_types0 = composites; prog_comp_env0 = PTree.empty }

(** val return_tcp1 : program0 res **)

let return_tcp1 =
  transf_beepl_program_csyntax return_example1
