...
Module Info.
  ...
  Definition source_file := "compiler_test/globvar2.c".
End Info.

Definition _BankAccount : ident := $"BankAccount".
...
Definition _accNum : ident := $"accNum".
Definition _argc : ident := $"argc".
Definition _argv : ident := $"argv".
Definition _balance : ident := $"balance".
...
Definition v___stringlit_1 := {|
  ...
  gvar_init := (Init_int8 (Int.repr 84) :: Init_int8 (Int.repr 104) ::
                ...
                Init_int8 (Int.repr 98) :: Init_int8 (Int.repr 97) ::
...
Definition v_a := {|
  gvar_info := (Tstruct _BankAccount noattr);
...
(Ssequence
  (Ssequence
    (Sdo (Eassign
           (Efield
             (Evalof (Evar _a (Tstruct _BankAccount noattr))
               (Tstruct _BankAccount noattr)) _accNum tint)
           (Eval (Vint (Int.repr 123)) tint) tint))
    (Ssequence
      (Sdo (Eassign
             (Efield
               (Evalof (Evar _a (Tstruct _BankAccount noattr))
                 (Tstruct _BankAccount noattr)) _balance tdouble)
             (Eval (Vfloat (Float.of_bits (Int64.repr 4652007308841189376))) tdouble)
             tdouble))
      (Ssequence
        (Sdo (Ecall
               (Evalof
...
(Composite _BankAccount Struct
   (Member_plain _accNum tint :: Member_plain _balance tdouble :: nil)
   noattr :: nil).
...
((___compcert_va_int32,
 ...
 (_printf,
   Gfun(External (EF_external "printf"
     ...
     {|cc_vararg:=(Some 1); cc_unproto:=false; cc_structret:=false|})) ::
 (_a, Gvar v_a) :: (_main, Gfun(Internal f_main)) :: nil).

Definition public_idents : list ident :=
(_main :: _a :: _printf :: ___builtin_debug :: ___builtin_fmin ::
