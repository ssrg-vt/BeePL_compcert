...
Module Info.
  ...
  Definition source_file := "compiler_test/bad_programs/volatile1.c".
End Info.
...
Definition _modify : ident := $"modify".
...
Definition f_modify := {|
  ...
  fn_params := ((_ptr, (tptr (tvolatile tint))) :: nil);
  fn_vars := nil;
  fn_body :=
(Sdo (Eassign
       (Ederef
         (Evalof (Evar _ptr (tptr (tvolatile tint))) (tptr (tvolatile tint)))
         (tvolatile tint)) (Eval (Vint (Int.repr 100)) tint)
       (tvolatile tint)))
|}.
...
Definition f_main := {|
  ...
  fn_callconv := cc_default;
  fn_params := nil;
  fn_vars := ((_v, (tvolatile tint)) :: nil);
  fn_body :=
(Ssequence
  (Ssequence
    (Sdo (Eassign (Evar _v (tvolatile tint)) (Eval (Vint (Int.repr 42)) tint)
           (tvolatile tint)))
    (Ssequence
      (Sdo (Ecall
             (Evalof
               (Evar _modify (Tfunction (Tcons (tptr (tvolatile tint)) Tnil)
                               tvoid cc_default))
               (Tfunction (Tcons (tptr (tvolatile tint)) Tnil) tvoid
                 cc_default))
             (Econs
               (Eaddrof (Evar _v (tvolatile tint)) (tptr (tvolatile tint)))
               Enil) tvoid))
...
((___compcert_va_int32,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=(Some 1); cc_unproto:=false; cc_structret:=false|})) ::
 (_modify, Gfun(Internal f_modify)) :: (_main, Gfun(Internal f_main)) :: nil).

Definition public_idents : list ident :=
(_main :: _modify :: ___builtin_debug :: ___builtin_fmin ::
