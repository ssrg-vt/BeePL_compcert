...
Module Info.
  ...
  Definition source_file := "compiler_test/bad_programs/volatile2.c".
End Info.
...
Definition _read_from_volatile : ident := $"read_from_volatile".
Definition _shared_var : ident := $"shared_var".
...
Definition v_shared_var := {|
  gvar_info := (tvolatile tint);
  gvar_init := (Init_int32 (Int.repr 10) :: nil);
  gvar_readonly := false;
  gvar_volatile := true
|}.

Definition f_read_from_volatile := {|
  fn_return := tvoid;
  fn_callconv := cc_default;
  fn_params := nil;
  fn_vars := ((_ptr, (tptr (tvolatile tint))) :: (_value, tint) :: nil);
  fn_body :=
(Ssequence
  (Sdo (Eassign (Evar _ptr (tptr (tvolatile tint)))
         (Eaddrof (Evar _shared_var (tvolatile tint))
           (tptr (tvolatile tint))) (tptr (tvolatile tint))))
  (Ssequence
    (Sdo (Eassign (Evar _value tint)
           (Evalof
             (Ederef
               (Evalof (Evar _ptr (tptr (tvolatile tint)))
                 (tptr (tvolatile tint))) (tvolatile tint)) (tvolatile tint))
           tint))
    (Sdo (Ecall
           (Evalof
             ...
             (Tfunction (Tcons (tptr tschar) Tnil) tint
               {|cc_vararg:=(Some 1); cc_unproto:=false; cc_structret:=false|}))
           (Econs
             (Evalof (Evar ___stringlit_1 (tarray tschar 16))
               (tarray tschar 16))
             (Econs (Evalof (Evar _value tint) tint) Enil)) tint))))
|}.
...
(Ssequence
  (Ssequence
    (Sdo (Ecall
           (Evalof
             (Evar _read_from_volatile (Tfunction Tnil tvoid cc_default))
...
((___compcert_va_int32,
 ...
 (_printf,
   Gfun(External (EF_external "printf"
     ...
     {|cc_vararg:=(Some 1); cc_unproto:=false; cc_structret:=false|})) ::
 (_shared_var, Gvar v_shared_var) ::
 (_read_from_volatile, Gfun(Internal f_read_from_volatile)) ::
 (_main, Gfun(Internal f_main)) :: nil).

Definition public_idents : list ident :=
(_main :: _read_from_volatile :: _shared_var :: _printf ::
