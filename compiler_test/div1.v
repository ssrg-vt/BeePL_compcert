...
Module Info.
  ...
  Definition source_file := "compiler_test/div1.c".
...
Definition f_div := {|
  ...
  fn_params := ((_x, tshort) :: (_y, tshort) :: nil);
  fn_vars := nil;
  fn_temps := nil;
  fn_body :=
(Sreturn (Some (Ebinop Odiv (Etempvar _x tshort) (Etempvar _y tshort) tint)))
|}.
...
Definition f_main := {|
  ...
  fn_params := nil;
  fn_vars := nil;
  fn_temps := ((_r, tshort) :: (_t'1, tshort) :: nil);
  fn_body :=
(Ssequence
  (Ssequence
    (Ssequence
      (Scall (Some _t'1)
        (Evar _div (Tfunction (Tcons tshort (Tcons tshort Tnil)) tshort
                     cc_default))
        ((Econst_int (Int.repr 8) tint) :: (Econst_int (Int.repr 2) tint) ::
         nil))
      (Sset _r (Ecast (Etempvar _t'1 tshort) tshort)))
    (Ssequence
      (Scall None
        (Evar _printf (Tfunction (Tcons (tptr tschar) Tnil) tint
                        {|cc_vararg:=(Some 1); cc_unproto:=false; cc_structret:=false|}))
        ((Evar ___stringlit_1 (tarray tschar 3)) :: (Etempvar _r tshort) ::
         nil))
...
((___compcert_va_int32,
 ...
 (_printf,
   Gfun(External (EF_external "printf"
     ...
     {|cc_vararg:=(Some 1); cc_unproto:=false; cc_structret:=false|})) ::
 (_div, Gfun(Internal f_div)) :: (_main, Gfun(Internal f_main)) :: nil).

Definition public_idents : list ident :=
(_main :: _div :: _printf :: ___builtin_debug :: ___builtin_fmin ::
