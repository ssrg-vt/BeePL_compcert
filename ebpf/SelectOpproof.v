...
Variable default_imm: comparison -> int -> condition.
Variable default: comparison -> condition.
...
Hypothesis sem_default_imm: forall c v n, sem c v (Vint n) = Val.of_optbool (eval_condition (default_imm c n) (v :: nil) m).
Hypothesis sem_default: forall c v n, sem c v n = Val.of_optbool (eval_condition (default c) (v :: n :: nil) m).

Lemma eval_compimm:
  forall le c a n2 x,
  eval_expr ge sp e m le a x ->
  exists v, eval_expr ge sp e m le (compimm default_imm default intsem c a n2) v
...
(* ne cmp *)
  - InvEval. inv H. simpl in H5. inv H5.
    ...
    + destruct (Int.eq_dec n2 Int.one). subst n2. TrivialExists.
      ...
      rewrite sem_undef; auto.
(* default *)
  - destruct (Int.ltu n2 (Int.repr 16)).
    + TrivialExists. simpl. rewrite sem_default_imm. auto.
    + eexists. split. repeat econstructor; eapply H.
      rewrite sem_default. auto.
Qed.
...
Lemma eval_compimm_swap:
  forall le c a n2 x,
  eval_expr ge sp e m le a x ->
  exists v, eval_expr ge sp e m le (compimm default_imm default intsem (swap_comparison c) a n2) v
